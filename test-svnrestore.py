...
def svn_repo_restore(repo_dump_path):
    ...
    try:
        ...
        newest_repo_dump_file = sorted(repo_backup_path_list, reverse=True)[0]
...
if __name__ == "__main__":
    ...
    repo_backup_path = "E:\svn_backup\\boss"
    ...
    file_list = os.listdir(repo_backup_path)    
    for item in file_list:
        file_full_path = os.path.join(repo_backup_path, item)
        if not os.path.isdir(file_full_path):
            repo_backup_file_list.append(file_full_path)
            print "dump file is <%s>" % (file_full_path)
    
    #######################################################################
    # Step 3. restore repository from dump file 

    newest_repo_dump_file = sorted(repo_backup_file_list, reverse=True)[0]
    print "newest dump file = %s" % newest_repo_dump_file
